...
## - 2026-09-14 (Phase 12: Physical Measurement Contract & Calibration Readiness — hardware parked)
### Added
- **`calibration/contract.py`** — the measured-observation contract: `check_observation()` issues explicit VALID / STALE / UNAVAILABLE / INVALID verdicts with reasons (finite + strictly positive distance; six-value provenance taxonomy; UNAVAILABLE/STALE labels can never carry a value; finite non-negative timestamp; future timestamps INVALID; staleness window default mirroring `MEASUREMENT_STALE_S`); `calibration_eligibility()` gates fit input to usable-value + independent-ground-truth pairs with per-record exclusion reasons; `accept_dataset()` separates INVALID-AUDIT-STATE (overlap / unknown provenance / unfrozen protocol → `DatasetAcceptanceError`) from INVALID-FOR-FIT (stale / unavailable / missing truth / invalid numerics → reported, never silently dropped) with an optional EXPERIMENT-TARGET coverage gate; `provenance_invariant_report()` machine-checks invariants A–G; `software_calibration_readiness()` exercises the ten software capabilities in-memory and reports the two gates explicitly separate (SOFTWARE readiness PASS ≠ hardware validation).
- **`measurement_quality_summary()`** (`evaluation/reporting.py`): deterministic counts per provenance, valid/invalid split, truth coverage, calibrated-label counts — MEASURED and SIMULATED accuracy never combined into a physical-accuracy claim.
- **Fit hardening** (`calibration/models.py`): non-finite (nan/inf) values are now explicitly rejected by `_fit_rows` in addition to the usability gate (inf > 0 previously slipped into the least-squares fit; found and fixed by the Phase 12 contract tests).
- **`tests/test_measurement_contract.py`**: 67 hardware-free tests — all provenance invariants A–G, stale boundary (exactly-within VALID / exactly-beyond STALE), future/non-finite timestamps, zero/negative/non-finite distances, provider-contract modes via `ScriptedDepthProvider`, DistanceFusion contract for every provenance state (incl. stale value withheld and OLD-timestamp demotion), calibration-provenance derivation, raw-record immutability (incl. deep snapshot across fit/diagnostics/report/manifest/rehydrate), eligibility exclusions, planned-target vs independent truth, split-leakage rejection, provenance survival through JSON + manifest round-trips, full-pipeline provenance flow (provider → fusion → record → capture → derived → calibration → report), quality summary, software readiness PASS with hardware gate BLOCKED, and SIMULATED-never-relabeled structural proofs.

### Verified
- 552/552 hardware-free pytest suite (485 pre-existing + 67 contract). Contract benchmark (10k records): check_observation ≈ 0.36 µs/record, eligibility 4.3 ms, quality summary 1.4 ms, accept_dataset 3.0 ms.
- SOFTWARE-VALIDATED / SIMULATION-VALIDATED only. **Software calibration readiness: PASS. OAK-D hardware checkpoint (Phase 9 Checkpoint A): BLOCKED. Hardware untouched** — actual stereo measurement, USB 3.x, physical accuracy, real-world calibration, environmental robustness, generalization, and safety remain NOT VALIDATED.

---

## - 2026-09-14 (Phase 11: Synthetic End-to-End Calibration Campaign — hardware parked)
### Added
- **`calibration/campaign.py`** — deterministic synthetic end-to-end campaign over the REAL Phase 10 capture protocol: known generating model `predicted = ground_truth × 1.10 + 0.08 + jitter(frame_id)` (pure arithmetic, no RNG), 3 sim_* scenes × 6 experiment targets × 2 repetitions (36 virtual slots), planned ABORT/resume slot (superseded attempt preserved with sentinel records + distinct observation identities, excluded from datasets; resumed attempt enters exactly once), one INCOMPLETE capture (stays UNASSIGNED + visible, carries a deliberately injected outlier), one untruthed slot (ground truth stays `None`, never 0), target vs ground truth kept distinct (2.00 m vs 2.01/2.02/2.03 m), raw records byte-stable through truth attachment/fit/freeze/report, protocol-level immutable CALIBRATION/VALIDATION split, baseline-first metrics, candidate fit on calibration data ONLY, FrozenCalibrationModel → validation metrics, coverage acceptance over both splits, dataset freeze, deterministic report + Markdown rendering.
- **Acceptance gates** (`AcceptanceGate`, `coverage_acceptance`) derived from MEASURED metrics only: split disjoint, baseline validation MAE > 0, calibrated MAE < baseline, calibrated RMSE < baseline, calibrated MAE within documented tolerance, EXPERIMENT-TARGET coverage complete in both splits. A deliberately wrong model (identity on biased data) is rejected; a zero-improvement fixture honestly fails the baseline-positive gate. No "calibration always passes" flag.
- **Manifest round-trip hardening** (`calibration/capture.py`, `calibration/protocol.py`): rehydration now restores the serialized ground-truth audit AND persisted derived counts (`record_count_audit` / `missing_truth_audit`) so audit copies preserve what each capture held; `missing_truth()` reports the audit value on rehydrated captures instead of fabricating 0; rehydrated protocols remain FROZEN.
- **`tests/test_synthetic_calibration_campaign.py`**: 41 hardware-free tests — determinism, virtual-slot completeness, capture lifecycle, abort preserved/resumed/superseded/excluded, incomplete stays incomplete, missing truth None, target ≠ truth, raw immutability, split disjointness, overlap rejection, fit-isolation (perturbed validation data cannot alter fitted parameters), baseline/calibrated metrics, validation improvement + tolerance, fitted-parameter recovery of the known bias, coverage enforcement, repeatability/outlier diagnostics, manifest round-trip + frozen immutability + post-freeze rejection, report determinism, acceptance pass/fail, no-physical-claims guard, and the AST no-navigation-imports guard.

### Verified
- 485/485 hardware-free pytest suite (444 pre-existing + 41 campaign). Canonical campaign (~30 ms end-to-end): affine selected; baseline validation MAE 0.3003 m → calibrated 0.0019 m (99.4% improvement); fitted inverse (0.909165, −0.072874) vs theoretical (0.909091, −0.072727); scaled 144-slot campaign 30.6 ms (verify_split 2.3 ms, manifest 2.2 ms, manifest JSON 6.9 ms/147 KiB).
- SOFTWARE-VALIDATED / SIMULATION-VALIDATED only. **Real OAK-D hardware validation remains blocked (Phase 9 Checkpoint A); synthetic success does not validate physical distance accuracy, calibration generalization, system behavior, or safety.**

---

## - 2026-09-14 (Phase 10 Data-Collection Protocol: Controlled Capture + Experiment Manifests — hardware parked)
### Added
- **Capture state machine** (`calibration/capture.py`): one `Capture` per planned (target distance, repetition) slot with a deterministic lifecycle — `PLANNED → ACTIVE → {COMPLETED | INCOMPLETE | ABORTED}` (+ `PLANNED → ABORTED`); terminal states immutable, invalid transitions raise `CaptureStateError`, data/truth attachment outside ACTIVE raises `CaptureNotActiveError`. A documented completeness rule (≥ min observations, known identity, project-vocabulary provenance, finite timestamps) routes rule failures to **INCOMPLETE** — kept and visible, never silently discarded; ground truth is deliberately not required for capture completion. Raw `MeasurementRecord`s are stored verbatim and never mutated: truth application happens in a regenerated DERIVED view (`derived_records()`), so raw and derived layers stay separate. `attach_ground_truth()` audits every attachment (`GroundTruthEntry`: identity, value, operator source/note, and uncertainty **only when explicitly supplied**) and rejects non-positive truth; missing truth stays `None`, never 0. Target distance (plan) and ground truth (observed reference) remain distinct fields, never merged.
- **Experiment protocol** (`calibration/protocol.py`): `ExperimentProtocol` drives the plan over one or more scenes — virtual planned slots (scene × target × repetition; a slot with no attempt is PLANNED), append-only capture-attempt log, **resume-after-abort** creating a new numbered attempt (the aborted attempt is preserved untouched; only the LATEST terminal attempt feeds a dataset), explicit per-slot **CALIBRATION/VALIDATION assignment** (immutable — re-assignment to the other role rejected; terminal captures only; `UNASSIGNED` reported), `verify_split()` reusing `assert_disjoint` (overlap fails loudly, nothing auto-resolved), **dataset freeze** (`freeze()` → `ProtocolFrozenError` on any further mutation; idempotent), honest **completion report** (planned/completed/incomplete/aborted slots, exact missing-observation and missing-ground-truth counts, resumed slots, assignment coverage — `fully_complete` stays `False` while anything is missing), and a deterministic **manifest** (`sina-capture-manifest/1`: plan, scenes, per-capture lifecycle + truth audit, assignment table; `from_manifest()` rehydrates as a FROZEN audit copy; environment fields stay `None` unless operator-supplied). Reuses `MeasurementRecord`, `ExperimentSession`, `TargetDistancePlan`, and the Phase 10-preparation experiment layer verbatim — no duplicated models; an AST-level test enforces no navigation imports.
- **`tests/test_capture_protocol.py`**: 46 hardware-free tests, all fixtures SIMULATED (`sim_*` scenes) — happy path, invalid/terminal/no-op transitions, records-require-ACTIVE, interrupted ≠ completed, completeness rule (no-records/bad-provenance/min-observations → INCOMPLETE), plan integration and out-of-plan rejection, repetition recoverability, raw preservation under truth attachment, audit entries incl. never-invented uncertainty, non-positive truth rejection, missing truth stays None, target vs ground-truth separation, terminal-only + immutable assignment, disjoint split verification, overlap rejection, abort/resume (aborted attempt preserved, cannot start over completed/active/incomplete), freeze blocking all four mutation paths (idempotent, reads still work), frozen-calibration workflow over protocol datasets, honest completion (empty/partial/missing-truth/aborted/resumed), manifest determinism + unknown-metadata-stays-None + frozen round trip, and the AST source guard.

### Verified
- 444/444 hardware-free pytest suite (398 pre-existing + 46 capture protocol). 160-slot synthetic benchmark: full capture run ≈ 2.8 ms, verify_split ≈ 2.3 ms, completion ≈ 3.5 ms, manifest ≈ 1.7 ms, to_json ≈ 5.9 ms (189 KB), from_manifest < 1 ms.
- SOFTWARE-VALIDATED / SIMULATION-VALIDATED only. **Real OAK-D capture remains blocked pending Phase 9 hardware validation**; no measurement, calibration-generalization, system, or safety claims.

---

## - 2026-09-14 (Phase 10 Preparation: Calibration Experiment Framework — hardware parked)
### Added
- **`calibration/` package** — the gated ground-truth dataset + calibration experiment framework, consuming `MeasurementRecord` verbatim (no duplicate representation) and `evaluation.metrics` math (no duplicated formulas):
  - `session.py`: `ExperimentSession` metadata (device, DepthAI version, resolutions, FPS, scene, lighting, surfaces, operator notes) — every field optional, unknown values stay `None`, unknown fields rejected on deserialize. No invented metadata.
  - `plan.py`: `TargetDistancePlan` — deterministic capture schedule of experiment target distances × repetitions. Targets are labeled EXPERIMENT TARGETS, never navigation/safety thresholds; an AST-level test enforces the package imports nothing from `navigation`.
  - `coverage.py`: distance/scene/label/track coverage; requested targets reported SEPARATELY from observed ground truth (nominal placement ≠ truth); availability/invalid-rate breakdown by distance, label, scene, and provenance, reusing `is_valid_record`.
  - `diagnostics.py`: repeatability per target/label/scene (groups below `min_samples` flagged `insufficient` with no statistics); descriptive outlier report (distributions, largest errors, per-bin/per-label means — listing only); `filter_outliers()` fully auditable (rule, counts, before/after MAE; raw data never mutated, never silently applied).
  - `models.py`: minimal deterministic calibration interface — `IdentityCalibration` (the uncalibrated baseline) and `AffineCalibration` (closed-form least squares; no ML). `fit()` enforces minimum-sample coverage loudly (`CalibrationError`, §12) and returns a `FrozenCalibrationModel` exposing transform only — the validation stage is structurally unable to refit (§20). Fitted transforms map provenance to explicit `MEASURED_CALIBRATED`/`SIMULATED_CALIBRATED` labels (never back to `MEASURED`); `compare_candidates()` selects by lowest MAE on CALIBRATION data only (tie → fewer parameters → first registered), with validation data structurally absent from selection (§19).
  - `experiment.py`: `CalibrationExperiment.run()` — assert_disjoint BEFORE fitting (overlap fails loudly, no records removed); raw baseline metrics computed and reported FIRST and ALWAYS (§10); `CalibratedObservation` preserves raw AND calibrated values + provenances side by side (§8); inputs never mutated; fully deterministic.
  - `reporting.py`: deterministic 16-section JSON experiment report (calibration performance and validation performance as separate, labeled sections) + Markdown renderer consuming structured results only.
- **`tests/test_calibration_framework.py`**: 55 hardware-free tests, all fixtures SIMULATED (`sim_*` scenes) — session round-trips/rejections, plan validation, coverage vs requested targets, per-dimension availability, repeatability with insufficient-group flagging, outlier listing + auditable filtering (incl. non-mutation), affine recovering a known synthetic bias (scale 1.1) exactly, identity winning the documented tie-break on unbiased data, loud insufficient-sample failures, frozen-model no-refit (AttributeError), disjointness-before-fitting (overlap raises), raw-record immutability through the experiment, identity-only run == baseline, per-run determinism, report sections/determinism/classification honesty, and the AST source guard against navigation imports.

### Verified
- 398/398 hardware-free pytest suite (343 pre-existing + 55 calibration). 9,984-record synthetic benchmark: fit+selection+evaluation ≈ 92 ms, report ≈ 36 ms, diagnostics ≤ 10 ms each; affine recovered the known simulated bias to MAE ≈ 0 on held-out data (SIMULATION-VALIDATED only).
- SOFTWARE-VALIDATED / SIMULATION-VALIDATED only. **Real OAK-D measurement calibration remains blocked pending hardware validation** (Phase 9 Checkpoint A not performed); no distance accuracy, generalization, navigation, or safety claims.
